...
exports.shortcut = {
	...
	list: () => {
		...
		apps.forEach(app => {
			console.log(
				`ID: ${'\x1b[36m' + id + '\x1b[0m'}, App: ${
					'\x1b[36m' + app.name + '\x1b[0m'
				}, Location: ${'\x1b[36m' + app.location + '\x1b[0m'}`
			)
			id++
		})
		id = 0
...
exports.start = name => {
	...
	if (Number.isInteger(Number(name))) {
		if (
			apps[Number(name)].location.substr(0, 4) === 'http' ||
			apps[Number(name)].location.substr(0, 3) === 'www'
		) {
			console.log('\x1b[36m%s\x1b[0m', 'Opening webpage...')
			opn(apps[Number(name)].location)
		} else if (
			apps[Number(name)].location.substr(0, 7) === 'dir:///' ||
			apps[Number(name)].location.substr(0, 6) === 'file://'
		) {
		...
		apps.forEach(app => {
			if (app.name === name) {
				if (
					app.location.substr(0, 4) === 'http' ||
					app.location.substr(0, 3) === 'www'
				) {
					console.log('\x1b[36m%s\x1b[0m', 'Opening webpage...')
					opn(app.location)
				} else if (
					app.location.substr(0, 7) === 'dir:///' ||
					app.location.substr(0, 6) === 'file://'
				) {
					console.log('\x1b[36m%s\x1b[0m', 'Opening directory...')
